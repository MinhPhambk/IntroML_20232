...
def cpu_bot_random(p_state, per):
...
def cpu_run_one_game(p_main, p_o, per, print_mode = False):
    env = init_env()
    ...
    while _cc <= NUMBER_COLS * NUMBER_ROWS:            
        ...
        if (print_mode):
            ...
            for i in range(NUMBER_ROWS):
                for j in range(NUMBER_COLS):
                    if (env[convert_to_1D(i, j)] == 0):
                        print('~', ' ', sep = '', end = '')
                    elif (env[convert_to_1D(i, j)] == 1):
                        print('X', ' ', sep = '', end = '')
                    elif (env[convert_to_1D(i, j)] == 2):
                        print('O', ' ', sep = '', end = '')
                print('\n')
...
def cpu_run_n_game(p0, p1, per, num_game, print_mode = False):
    ...
    for _n in range(num_game):
        ...
        if (first == 0):
            winner, per = cpu_run_one_game(p0, p1, per, print_mode)
        else:
            winner, per = cpu_run_one_game(p1, p0, per, print_mode)
